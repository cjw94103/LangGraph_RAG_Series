...
if process:
    ...
    ## 시스템 프롬프트
    st.session_state.llm_sys_prompt = load_string_from_txt("./sys_prompt_hub/04_self_rag/02_llm_answer_system_message.txt")
    ...
    graph_ins = SELFRAG(retriever=st.session_state.retriever, top_k=st.session_state.top_k)
...
if question := st.chat_input():
    ...
    with st.chat_message("assistant"):
        ...
        answer = st.session_state.app.invoke(inputs, config)

        # output_str = ""
        # def stream_data():
        #     global output_str
        #     for chunk_msg, metadata  in st.session_state.app.stream(inputs, config, stream_mode="messages"):
        #         print(metadata["langgraph_step"])
        #         if metadata["langgraph_node"] == "make_chat_history":
