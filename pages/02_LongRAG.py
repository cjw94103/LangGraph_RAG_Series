...
from core.long_rag import LongRAG
...
with st.sidebar:
    ...
    top_k = st.slider("Top-k 파라미터 (문서 검색 수) 설정", min_value=1, max_value=10, value=7, step=1)
    st.divider()
...
if process:
    ...
    ## 시스템 프롬프트
    st.session_state.llm_sys_prompt = load_string_from_txt("./sys_prompt_hub/02_long_rag/04_02_llm_answer_system_message.txt")
    ...
    # dense retriever
    dense_retriever = make_uploadfile_to_dense_retriever(filepaths=filepaths,
    ...
    graph_ins = LongRAG(dense_retriever=st.session_state.dense_retriever, top_k=st.session_state.top_k)
